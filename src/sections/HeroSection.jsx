...
import { FaGithub, FaLinkedin, FaTwitter, FaFacebook, FaEnvelope, FaReact, FaNodeJs, FaPython } from "react-icons/fa";
...
const projectStats = [
  { title: "HealTogether", stars: 120, contributors: 5, url: "#" },
  { title: "MindTrack", stars: 85, contributors: 3, url: "#" },
  { title: "WellnessAI", stars: 95, contributors: 4, url: "#" },
];
...
const Dashboard = () => {
  ...
  const Sidebar = () => (
    ...
    >
      ...
      <div className="flex gap-4 mt-auto">
        ...
        <a href="https://www.facebook.com/justcodeitnow" target="_blank" rel="noopener noreferrer">
          <FaFacebook className="hover:text-indigo-500 transition cursor-pointer" />
        </a>
  ...
  const MobileTabs = () => (
    <div className="fixed top-0 left-0 right-0 z-40 bg-gray-800 flex justify-around p-2 shadow-md md:hidden">
      {tabs.map((tab) => (
        <button
          key={tab}
          onClick={() => handleTabChange(tab)}
          className={`py-2 px-4 rounded-lg font-medium transition-colors duration-300 ${
            activeTab === tab ? "bg-indigo-500 text-white" : "text-gray-300 hover:bg-indigo-700"
          }`}
        >
          {tab.charAt(0).toUpperCase() + tab.slice(1)}
        </button>
  ...
  const Projects = () => (
    ...
    >
      {projectStats.map((proj) => (
        <div key={proj.title} className="bg-gray-700 rounded-2xl p-4 sm:p-6 shadow-lg flex flex-col justify-between">
          <div>
          ...
          <a href={proj.url} target="_blank" rel="noopener noreferrer" className="text-indigo-500 hover:underline mt-2 text-sm md:text-base">
            View Repo
          </a>
        </div>
      ))}
  ...
  const Contact = () => (
    ...
    >
      ...
      <a href="mailto:jonipassion@gmail.com" className="text-indigo-500 hover:underline text-sm md:text-base">
        jonipassion@gmail.com
      </a>
    </motion.div>
  );
  ...
  return (
    <div className="min-h-screen flex bg-gray-900 text-white">
      ...
      {/* Mobile Tabs */}
      {isMobile && !sidebarOpen && <MobileTabs />}

      {/* Main Content */}
      <main className={`flex-1 p-4 sm:p-6 md:p-10 transition-all ${sidebarOpen ? "ml-64" : "ml-0"} mt-${isMobile && !sidebarOpen ? "12" : "0"}`}>
